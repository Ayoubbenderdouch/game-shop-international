  ...
  return (
    <div className="relative group inline-flex items-center">
      ...
      <div
        className={`relative ${
          sizeClasses[size]
        } rounded transition-all duration-300 border-2 
        ${
          checked
            ? variant.bg + " border-transparent"
            : "bg-transparent border-gray-500 " + variant.hover
        }`}
      >
        {/* Checkmark icon with animation */}
        <svg
          className={`absolute inset-0 w-full h-full text-white stroke-white fill-none stroke-2 
      ...
      {/* Focus/active ripple effect */}
      <div
        className={`absolute ${
          sizeClasses[size] === "w-4 h-4"
            ? "w-8 h-8"
            : sizeClasses[size] === "w-5 h-5"
            ? "w-10 h-10"
            : "w-12 h-12"
        } 
        -translate-x-1/2 -translate-y-1/2 rounded-full pointer-events-none 
        transition-opacity duration-500 opacity-0 
        ${
          variant.ripple
        } scale-0 group-active:scale-100 group-active:opacity-70`}
      ></div>
    </div>
  );
};
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="mb-8 bg-gradient-to-l from-dark-card to-dark-lighter p-6 rounded-lg shadow-lg">
        ...
        {selectedCategory &&
          categoriesArray.find((c) => c.id === selectedCategory) && (
            <div className="mt-2 text-primary">
              تصفية حسب:{" "}
              {categoriesArray.find((c) => c.id === selectedCategory)?.name}
            </div>
          )}
        {onlyOnSale && (
          <div className="mt-2 text-secondary">
            <span className="bg-secondary/20 text-secondary px-3 py-1 rounded-full text-sm">
              العروض والتخفيضات فقط
            </span>
          </div>
        )}
      </div>
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        >
          <Card
            padding="lg"
            className="mb-4 sticky top-20 border border-dark-lighter shadow-xl"
          >
            ...
            {/* Category Filter */}
            <div className="mb-6">
              <h3 className="font-bold mb-3 text-text-light border-r-4 border-primary pr-2">
                تصفية حسب التصنيف
              </h3>
              <div className="space-y-2 max-h-64 overflow-y-auto pr-1 thin-scrollbar">
                {categoriesLoading ? (
                  ...
                  categoriesArray.map((category) => (
                    <div
                      key={category.id}
                      className={`flex items-center hover:bg-dark-lighter/30 p-2 rounded-md transition-all duration-300 
                      ${
                        selectedCategory === category.id
                          ? "bg-dark-lighter/40 shadow-sm"
                          : ""
                      }`}
                    >
                      <EnhancedCheckbox
                      ...
                      <label
                        ...
                        hover:text-primary cursor-pointer transition-colors mr-3`}
                      >
                        {category.name}
                      </label>
                    </div>
            ...
            <div className="mb-6">
              ...
              <form onSubmit={handleSearchSubmit} className="relative">
                <input
                  ...
                  className="w-full px-4 py-3 bg-dark-lighter rounded-md text-text-light 
                  border border-dark-lighter focus:outline-none focus:ring-2 focus:ring-primary 
                ...
                <div className="flex justify-between">
                  <div className="flex items-center">
                    ...
                    <input
                      ...
                      className="w-24 px-2 py-2 bg-dark text-text-light rounded border 
                      border-dark-lighter focus:outline-none focus:ring-2 
            ...
            <div className="mb-6">
              ...
              <div
                className={`flex items-center bg-dark-lighter/30 p-3 rounded-lg 
                transition-all duration-300 ${
                  onlyOnSale ? "bg-secondary/10 border-r border-secondary" : ""
                }`}
              >
                <EnhancedCheckbox
                  id="on_sale"
                ...
                <label
                  htmlFor="on_sale"
                  className={`mr-3 cursor-pointer font-medium transition-colors duration-300
                  ${
                    onlyOnSale
                      ? "text-secondary"
          ...
          <div className="mb-6 flex flex-col sm:flex-row justify-between items-center bg-dark-card border border-dark-lighter rounded-lg p-4 shadow-lg">
            ...
            <div className="flex space-x-4">
              ...
              <div className="relative">
                ...
                <div
                  id="sort-dropdown"
                  className="absolute left-0 mt-2 w-48 bg-dark border-2 border-dark-card rounded-md shadow-xl z-50 hidden transform transition-transform duration-300 origin-top-right"
                ...
                >
                  <div className="py-2 bg-dark-card rounded-md">
                    ...
                    <button
                      className={`block px-4 py-3 text-sm hover:bg-primary/20 w-full text-right transition-colors duration-300 ${
                        sortBy === "popularity_order"
